$(document).ready(function () {
   ...
   */
  $(
    'a[rel="lightbox"]:not(.nolightbox), a[href*=".jpg"]:not(.nolightbox), a[href*=".png"]:not(.nolightbox), a[href*=".gif"]:not(.nolightbox), a[href*=".jpeg"]:not(.nolightbox)',
  ).colorbox({
    photo: true,
 ...
 */
(function ($) {
  $.fn.intersect = function (settings) {
    var settings = $.extend({}, $.fn.intersect.defaultSettings, settings);
    function checker(settings) {
      if ($.fn.intersect.isOnScreen(settings))
        settings.instance.addClass("intersect").removeClass("noIntersect");
      else settings.instance.removeClass("intersect").addClass("noIntersect");
    }
    this.each(function (i, elm) {
      settings.instance = $(elm);
      $.fn.intersect.checker(settings);
  ...
  $.fn.intersect.isOnScreen = function (settings) {
    var win = $(window);
    var viewport = { top: win.scrollTop(), left: win.scrollLeft() };
    viewport.right = viewport.left + win.width();
    viewport.bottom = viewport.top + win.height();
    var bounds = settings.instance.offset();
    bounds.right = bounds.left + settings.instance.outerWidth();
    bounds.bottom = bounds.top + settings.instance.outerHeight();
    return !(
  ...
  $.fn.intersect.checker = function (settings) {
    if ($.fn.intersect.isOnScreen(settings))
      settings.instance.addClass("intersect").removeClass("noIntersect");
    else settings.instance.removeClass("intersect").addClass("noIntersect");
  };
  $.fn.intersect.defaultSettings = {};
})(jQuery);
...
(function ($) {
  $.fn.backToTop = function (settings) {
    var settings = $.extend({}, $.fn.backToTop.defaultSettings, settings);
    settings = $.fn.backToTop.createMarkup(settings);
    settings.button.click(function (e) {
      e.preventDefault();
      $("html,body").stop().animate({ scrollTop: 0 }, settings.speed);
    });
    settings.button.fadeOut(0);
    $(window).scroll(function (e) {
      if ($(this).scrollTop() > settings.displayAt) settings.button.fadeIn();
      else settings.button.fadeOut();
    });
    return this;
  };
  $.fn.backToTop.createMarkup = function (settings) {
    settings.button = $(
      '<a href="#" class="backToTop">' + settings.text + "</a>",
    );
    $(settings.placement).append(settings.button);
    return settings;
  };
  $.fn.backToTop.defaultSettings = {
    placement: "body",
...
(function ($) {
  $.fn.fontResizer = function (settings) {
    var settings = $.extend({}, $.fn.fontResizer.defaultSettings, settings);
    settings.placement = $(this);
    settings = $.fn.fontResizer.createMarkup(settings);
    settings.targets =
      settings.targets.length > 0 ? settings.targets : $("body");
    settings.smaller.click(function (e) {
      e.preventDefault();
      settings.targets.each(function (i, elm) {
        currentSize = parseInt($(elm).css("font-size"));
        if (currentSize <= settings.min) return false;
        else $(elm).css({ "font-size": "-=" + settings.steps });
      });
    });
    settings.bigger.click(function (e) {
      e.preventDefault();
      settings.targets.each(function (i, elm) {
        currentSize = parseInt($(elm).css("font-size"));
        if (currentSize >= settings.max) return false;
        else $(elm).css({ "font-size": "+=" + settings.steps });
      });
    });
  };
  $.fn.fontResizer.createMarkup = function (settings) {
    var markup = $(
      '<ul class="fontResizer"><li><a href="#" class="smaller">' +
        settings.smallText +
        '</a></li><li><a href="#" class="bigger">' +
        settings.bigText +
        "</a></li></ul>",
    );
    settings.placement.append(markup);
    settings.smaller = $(".fontResizer .smaller");
    settings.bigger = $(".fontResizer .bigger");
    return settings;
  };
  $.fn.fontResizer.defaultSettings = {
    max: 20,
    min: 10,
    steps: 2,
    targets: {},
    bigText: "Schrift vergrößern",
    smallText: "Schrift verkleinern",
  };
})(jQuery);
...
(function ($) {
  $.fn.smartTitle = function (settings) {
    var settings = $.extend({}, $.fn.browserNotice.defaultSettings, settings);
    settings.elements = this;
    settings = $.fn.smartTitle.createMarkup(settings);
    settings.elements.each(function (i, elm) {
      ...
      $(elm).hover(
        function () {
          ...
          settings.instance
            ...
            .css({ display: "block" })
            .stop()
            .animate({ top: "-=10px", opacity: 1 }, settings.speed);
        ...
        function () {
          ...
          settings.instance
            .stop()
            .animate({ top: "+=5px", opacity: 0 }, settings.speed, function () {
              settings.instance.css({ top: 0, left: 0, display: "none" });
            });
  ...
  $.fn.smartTitle.position = function (settings) {
    ...
    settings.instance.css({ top: settings.top, left: settings.left });
    return settings;
  };
  $.fn.smartTitle.defaultSettings = { speed: 500 };
})(jQuery);
 ...
 */
(function ($) {
  $.fn.browserNotice = function (settings) {
    var settings = $.extend({}, $.fn.browserNotice.defaultSettings, settings);
    ...
    settings.name = M[0];
    settings.version = parseInt(M[1]);
    if (settings.createNotice) $.fn.browserNotice.createNotice(settings);
    return this;
  };
  $.fn.browserNotice.createNotice = function (settings) {
    var html = $(
      '<div class="noticeBar">Ihr Webbrowser (' +
        settings.name +
        " " +
        settings.version +
        ') ist nicht aktuell. Er enthällt bekannte Sicherheitslücken und könnte diese und weitere Webseiten nicht korrekt darstellen. <a href="https://web.archive.org/web/20171024222248/http://browsehappy.com/" target="_blank" title="Jetzt updaten">Lernen Sie aktuell zu bleiben!</a></div>',
    );
    var notice = false;
    if (settings.name == "Firefox" && settings.version < settings.firefox)
      notice = true;
    if (settings.name == "Chrome" && settings.version < settings.chrome)
      notice = true;
    if (settings.name == "MSIE" && settings.version < settings.msie)
      notice = true;
    if (settings.name == "Safari" && settings.version < settings.safari)
      notice = true;
    if (settings.name == "Opera" && settings.version < settings.opera)
      notice = true;
    if (notice) $("body").prepend(html);
  };
  $.fn.browserNotice.defaultSettings = {
    firefox: 17,
    chrome: 25,
    msie: 8,
    safari: 4,
    ...
    spin: function (t) {
      ...
      if (t) {
        ...
        f(r, {
          left:
            (n.left == "auto"
              ? l.x - a.x + (t.offsetWidth >> 1)
              : parseInt(n.left, 10) + s) + "px",
          top:
            (n.top == "auto"
              ? l.y - a.y + (t.offsetHeight >> 1)
              : parseInt(n.top, 10) + s) + "px",
        });
      }
    ...
    lines: function (t, e) {
      ...
      for (; r < e.lines; r++) {
        l = f(o(), {
          ...
          animation:
            i &&
            s(e.opacity, e.trail, a + r * e.direction, e.lines) +
              " " +
              1 / e.speed +
              "s linear infinite",
        });
